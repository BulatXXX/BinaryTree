class BinaryTree(var root: Node? = null) {
    ...
    /*
    Этот код рекурсивно просматривает дерево до тех пор,
    пока не найдет узел с заданным значением.

    Затем он применяет различные сценарии удаления
    в зависимости от того, сколько потомков имеет узел:

        - Если у узла нет потомков, он просто удаляется.
        - Если у узла есть только один потомок, он заменяется этим потомком.
        - Если у узла есть два потомка, значение узла заменяется на наименьшее значение из правого поддерева, а затем это наименьшее значение удаляется из правого поддерева.
